...
unsigned long long PC1(const char* source)
{
    ...
    static char perm[] = { 56,48,40,32,24,16, 8, 0,57,49,41,33,25,17, 9, 1,
    ...
    for (i = 0; i < 56; i++, tmp = tmp << 1LL)
        ...
        if ((*(unsigned long long*)source) & (1LL << perm[i])) res |= tmp;
    return res;
}
...
unsigned long long PC2()
{
    ...
    ​
        ...
        for (i = 0; i < 48; i++, tmp = tmp << 1LL) if (KEY.K & (1LL << perm[i])) res |= tmp;
...
unsigned long long IP(unsigned long long source)
{
    ...
    for (i = 0; i < 64; i++, tmp = tmp << 1LL) if (source & (1LL << perm[i])) res |= tmp;
    return res;
}
...
unsigned long long EX(unsigned source)
{
    char i;
    unsigned long long tmp = 1LL, res = 0LL;
    static char perm[] = { 31, 0, 1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 7, 8, 9,10,
...
{
    char i;
    unsigned tmp = 1, res = 0;
    static char perm[] = { 14, 6,19,20,28,11,27,16, 0,14,22,25, 4,17,30, 9,
                            1, 7,23,13,31,26, 2, 8,18,12,29, 5,21,10, 3,24 };



    for (i = 0; i < 32; i++, tmp = tmp << 1) if (source & (1 << perm[i])) res |= tmp;
    return res;
}
...
unsigned S(unsigned long long source)
{
    ...
    {
        z = (source >> (i * 6)) & 0x3F;
        res |= (perm[i][((z & 0x20) | ((z << 4) & 0x10) | (z >> 1) & 0x0F)] << (i * 4));
    }
    return res;
}
...
unsigned rotation(unsigned source, char c, char f)
{
    ...
    else   while (c-- > 0) source = ((source << 1) & 0x0FFFFFFF) | (source >> 27);
    return source;
}
...
unsigned long long EN(unsigned long long source)
{
    ...
    {
        Ln = R;
        KEY.B.C = rotation(KEY.B.C, i, 0);
        KEY.B.D = rotation(KEY.B.D, i, 0);
        R = L ^ P(S(EX(R) ^ PC2()));
        L = Ln;
    }
...
unsigned long long DE(unsigned long long source)
{
    ...
    while (--i >= 0)
    {
        ...
        R = L ^ P(S(EX(R) ^ PC2()));
        L = Ln;
        KEY.B.C = rotation(KEY.B.C, i, 1);
        KEY.B.D = rotation(KEY.B.D, i, 1);
    }
    return FP(((unsigned long long)L << 32) | R);
}
...
void DES_encrypt_str(const char* src, char* des, const char* key)
{
    ...
    KEY.K = PC1(key);
    for (i = 0; i < len; i += 8) *(unsigned long long*)(des + i) = EN(*(unsigned long long*)(src + i));
    des[len] = 0;
    return;
}
...
void DES_decrypt_str(const char* src, char* des, const char* key)
{
    int i, len = strlen(src);
    ...
    for (i = 0; i < len; i += 8) *(unsigned long long*)(des + i) = DE(*(unsigned long long*)(src + i));
    des[len] = 0;
    return;
}
...
int main()
{
    ...
    char encrypted_str[256], decrypted_str[256];

    int r = strlen(source_str) % 8;



    if (r) { printf("원본의 글자수를 %d개 줄이던지 %d개를 늘려주세요.\n", r, 8 - r); }
    else
    {
        printf("[원본] %s\n", source_str);
        puts_hex(source_str);
